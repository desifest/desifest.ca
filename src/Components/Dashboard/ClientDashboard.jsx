    ...
    <div className="flex h-screen bg-[#100422]">
      ...
      <div className="flex w-full flex-col md:hidden">
          ...
          <div className="border-b border-white/10 bg-[#0a0118] px-4 py-2 space-y-1">
            {TABS.map(({ key, label, icon: Icon }) => (
              <button key={key} onClick={() => { setActiveTab(key); setMobileMenuOpen(false) }}
                className={`flex w-full items-center gap-3 rounded px-3 py-2 text-left text-sm font-medium ${activeTab === key ? 'bg-[#D5FF00]/10 text-[#D5FF00]' : 'text-gray-300'}`}>
                <Icon className="h-4 w-4" /> {label}
              </button>
            ))}
        ...
        <div className="flex overflow-x-auto border-b border-white/10 bg-[#0a0118] px-2 md:hidden">
          {TABS.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setActiveTab(key)}
              className={`flex shrink-0 items-center gap-1.5 px-3 py-2.5 text-xs font-medium transition ${activeTab === key ? 'border-b-2 border-[#D5FF00] text-[#D5FF00]' : 'text-gray-300'}`}>
  ...
  return (
    <div className="max-w-3xl space-y-4">
      ...
      {bookings.length === 0 && !showNew ? (
        <p className="text-gray-300">No bookings yet. Create your first booking request!</p>
      ) : (
        bookings.map(b => (
...
function NewBookingForm({ authFetch, onDone }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded border border-white/10 bg-white/5 p-4 space-y-4">
      ...
      <div>
        <label className="mb-1 block text-xs text-gray-300">Artist</label>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs text-gray-300">Event Name</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-gray-300">Event Date</label>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs text-gray-300">Location</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-gray-300">Budget ($)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-gray-300">Description</label>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onDone} className="rounded border border-white/20 px-4 py-2 text-sm text-gray-300 hover:bg-white/5">Cancel</button>
      </div>
    </form>
  )
}
    ...
    <div className="max-w-2xl space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded border border-white/10 bg-white/5 p-4 space-y-4">
          {error && <p className="text-sm text-red-400">{error}</p>}
          <div>
            <label className="mb-1 block text-xs text-gray-300">Booking</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-gray-300">Rating</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-gray-300">Review</label>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="rounded border border-white/20 px-4 py-2 text-sm text-gray-300 hover:bg-white/5">Cancel</button>
          </div>
        </form>
      )}

      {reviews.length === 0 && !showForm ? (
        <p className="text-gray-300">No reviews yet.{bookings.length > 0 ? ' You can review completed bookings.' : ''}</p>
      ) : (
        reviews.map(r => (
          <div key={r.id} className="rounded border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-300">{r.artist_first_name} {r.artist_last_name}</p>
              </div>
            ...
            <p className="mt-2 text-xs text-gray-300">{new Date(r.created_at).toLocaleDateString()}</p>
          </div>
        ))
      )}
    </div>
...
function FavouritesSection({ authFetch }) {
  ...
  if (loading) return <div className="text-gray-300">Loading favourites...</div>

  return (
    <div className="max-w-2xl space-y-4">
      <h2 className="oswald-500 text-2xl uppercase">Favourite Artists</h2>
      {favourites.length === 0 ? (
        <p className="text-gray-300">No favourite artists yet. Browse artists to save your favourites.</p>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2">
          {favourites.map(f => (
            <div key={f.favourite_id} className="flex items-center gap-3 rounded border border-white/10 bg-white/5 p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {f.genre && <p className="text-xs text-gray-300">{f.genre}</p>}
              </div>
